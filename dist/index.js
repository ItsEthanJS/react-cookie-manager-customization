...
const setBlockingEnabled = (enabled) => {
...
const CookieManager = ({
  ...
  useEffect(() => {
    ...
    const maybeShow = async () => {
      ...
      const fetchWithTimeout = async (url2, ms) => {
      ...
      try {
        ...
        if (cached) {
          ...
          if (Date.now() - parsed.ts < 6 * 60 * 60 * 1e3) {
            if (!cancelled) {
      ...
      if (w && w[GEO_PROMISE_KEY]) {
        try {
          const show = await w[GEO_PROMISE_KEY];
          if (!cancelled) {
